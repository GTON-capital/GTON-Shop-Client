...
const VModelTest = getVModelComponent(CategoriesFilter, {
...
function createWrapper({
  propsData = { categories: getCategories() },
  slots = {},
} = {}) {
...
describe('CategoriesFilter', () => {
  ...
  it('should select a category by id if `selected` prop is given', async () => {
    wrapper = createWrapper({
      propsData: { selected: 1, categories: getCategories() },
    });
  ...
  it('should properly handle v-model', async () => {
    ...
    const categoriesFilter = wrapper.findComponent({
