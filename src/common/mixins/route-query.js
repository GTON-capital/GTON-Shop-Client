  ...
  return {
      ...
      $route: {
        async handler(value) {
          if (!this.__ignoreRouteChange) {
            ...
            let query;
            let transformCollectionQuery = await this.transformCollectionQuery(
              value.query,
              this.filters
            );
            ...
            if (isArray(transformCollectionQuery)) {
              ...
              typeof transformCollectionQuery.collections === 'string'
            ) {
      ...
      [propertyName](value) {
        ...
        if (!this.__ignorePropertyChange) {
          ...
          if ('collections' in value)
            query.collections = query.collections.map(item => item.value);
    ...
    methods: {
      ...
      async transformCollectionQuery(newValue, oldValue) {
        if (
          isArray(newValue.collections) &&
          !isObject(newValue.collections[0])
        ) {
          let res = Promise.all(
            newValue.collections.map(async newItem => {
              let item;
              if (oldValue.collections && isArray(oldValue.collections)) {
                item = oldValue.collections.find(
                  oldItem => oldItem.value === newItem
                );
                if (item === undefined) {
          ...
          if (oldValue.collections && isArray(oldValue.collections)) {
            item = oldValue.collections.find(
              oldItem => oldItem.value === newValue.collections
            );
            if (item === undefined) {
              let { name, contract } = await getCollection(
