<template>
  <div
    class="acurrencydropdown"
    :class="{ 'acurrencydropdown-noarrow': currencies.length < 2 }"
  >
    <a-dropdown-listbox
      v-bind="$attrs"
      :data="currencies"
    ...
    >
      ...
      <template #button-arrow>
        <span
          class="acurrencydropdown_arrow"
          :class="{ arrow_rotate: rotated }"
        >
  ...
  methods: {
    ...
    getSelectedTokenByAddress(tokenAddress) {
      ...
      if (currencies.length > 0) {
        return tokenAddress
          ? currencies.find(token =>
              compareAddresses(token.address, tokenAddress)
            )
          : currencies[0];
      }
