<template>
  <div class="abpricefield">
    ...
    >
      ...
      <template #suffix>
        <div class="abpricefield_price abpricefield_center">
          {{ to$(dValue) }}
        </div>
      </template>
    </f-input>
...
export default {
  ...
  computed: {
    cBalance() {
      return this.balance
        ? formatTokenValue(this.balance, this.token.decimals, 3)
        : 0;
    },
  },
  ...
  methods: {
    ...
    validator(value) {
      ...
      if (
        !isNaN(value) &&
        bToTokenValue(value, this.token.decimals).isGreaterThan(
    ...
    to$(value) {
      const { token } = this;
      const value$ = value
        ? toBigNumber(value).multipliedBy(token.price)
        : null;
