<template>
  <div class="apptextsearchresults">
    <div v-if="collections.length > 0">
      <h3 id="apptextsearchresults_collections">
        {{ $t('apptextsearchresults.collections') }}
      </h3>

      <ul aria-labelledby="apptextsearchresults_collections">
        <li
          v-for="collection in collections"
          :key="`${collection.contract}_atsr_col`"
        >
          <router-link
            :to="{
              name: 'explore',
              query: { collections: collection.contract },
            }"
    ...
    <div v-if="users.length > 0">
      <h3 id="apptextsearchresults_users">
        {{ $t('apptextsearchresults.accounts') }}
      </h3>
    ...
    <div v-if="tokens.length > 0">
      <h3 id="apptextsearchresults_tokens">
        {{ $t('apptextsearchresults.items') }}
      </h3>

      <ul aria-labelledby="apptextsearchresults_tokens">
        <li
          v-for="token in tokens"
          :key="`${token.contract}_${token.tokenId}_atsr_tok`"
        >
          <router-link
            :to="{
              name: 'nft-detail',
              params: { tokenContract: token.contract, tokenId: token.tokenId },
            }"
            class="apptextsearchresults_item"
          >
            <f-image
