  ...
  <f-form class="wrapstationform" :aria-label="$t('wrapStation')">
    ...
    <div class="center-h mat-5">
      <a-button
        ...
        @click.native="onWrapButtonClick"
      >
        {{ buttonLabel }}
      </a-button>
    </div>
...
export default {
  ...
  computed: {
    ...
    buttonLabel() {
      return this.wrap
        ? this.$t('wrapstationform.wrap')
        : this.$t('wrapstationform.unwrap');
    },
  },
  ...
  methods: {
    ...
    async setBalances() {
      ...
      const bMaxBalance = toBigNumber(this.ftmToken.balance).minus(
        bToTokenValue(1, this.ftmToken.decimals)
      );
      this.ftmToken.maxBalance = bMaxBalance.isLessThan(0)
        ? this.ftmToken.balance
        : toHex(bMaxBalance);
    ...
    setTx(type) {
      ...
      const amount = this.isMaxValue
        ? token.maxBalance
        : toHex(bToTokenValue(this.value, token.decimals));
      let tx = null;
