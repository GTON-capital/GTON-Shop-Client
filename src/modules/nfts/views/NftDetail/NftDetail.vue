  ...
  <div class="nftdetail container">
    <div class="nftdetail_main">
      <div class="nftdetail_media">
        <div
          v-if="token.imageMimetype && token.imageMimetype.startsWith('video/')"
          class="nftdetail_video"
        >
          <a-video
            :src="token.image"
            :poster="getImageThumbUrl(token.imageThumb)"
            loop
          />
        </div>
      ...
      <div class="nftdetail_product">
        <div class="nftdetail_infoWrap">
          ...
          <div class="nftdetail_status">
            <div class="nftdetail_owner">
              ...
              <router-link
                ...
                :aria-label="
                  `${$t('nftdetail.owned')} ${tokenOwnerName ||
                    tokenOwner.address}`
                "
              >
                <a-address
          ...
          <div
            v-if="userOwnsToken && token.contract"
            class="nftdetail_owneractions"
          >
            ...
            <template v-else>
              <nft-cancel-auction-button
                :token="token"
                :auction="auction"
                @tx-success="update"
              />
              <nft-update-auction-button
                v-if="!auctionHasFinished"
                :token="token"
                @tx-success="update"
              />
            </template>
    ...
    <div class="nftdetail_collection">
      <a-details strategy="create">
        <template #label>
          <div class="nftdetail_details_wrap">
            <h2>
              <app-iconset icon="collection" />
              {{ $t('nftdetail.fromCollection') }}
            </h2>
          </div>
        </template>
        <template>
...
export default {
  ...
  computed: {
    ...
    tokenHasListing() {
      return (
        !!this.listing.unitPrice &&
        !this.listing.closed &&
        this.listing.isActive
      );
    },
    ...
    tokenHasAuction() {
      ...
      return (
        this.token.hasAuction || (auction.contract ? !auction.closed : false)
      );
    },
  ...
  methods: {
    ...
    async update() {
      ...
      this.tokenOwner = await this.getTokenOwner(
        routeParams.tokenContract,
        routeParams.tokenId
      );
      this.token = await getToken(
        routeParams.tokenContract,
        toHex(routeParams.tokenId)
      );
    ...
    async loadAuction() {
      this.auction =
        (await getAuction(this.token.contract, this.token.tokenId)) || {};
    },
    ...
    async onWalletAddressChange() {
      ...
      if ($wallet.connected && $wallet.account) {
        this.userOwnsToken = compareAddresses(
    ...
    async getTokenOwner(tokenContract, tokenId) {
      const data = await getTokenOwnerships(
        tokenContract,
        tokenId,
        { first: 500 },
        true
    ...
    async setListing() {
      ...
      const data = await getTokenListings(token.contract, token.tokenId, {
        first: 200,
      });
