  ...
  <a-details-group class="nftdetailinfo" rounded>
    <a-details class="nftdetailinfo_block tes-3">
      ...
      <template>
        <div class="nftdetailinfo_row">
          <span>{{ $t('nftdetail.symbol') }}:</span
          ><span>{{ info.symbol }}</span>
        </div>
        <div v-if="info.royalty" class="nftdetailinfo_row">
          <span>{{ $t('nftdetail.royalty') }}:</span
          ><span>{{ info.royalty / 100 }}%</span>
        </div>
        <div v-if="info.feeRecipientUser" class="nftdetailinfo_row">
          ...
          <router-link
            :to="{
              name: 'account',
              params: { address: info.feeRecipientUser.address },
            }"
          >
            <a-address
    ...
    <a-details class="nftdetailinfo_block tes-3" v-if="info.collection">
      <template #label>
        <div class="nftdetail_details_wrap">
          <h2>
            <app-iconset icon="about" /> {{ $t('nftdetail.about') }}
      ...
      <template>
        <div class="nftdetailinfo_row">
          ...
          <router-link
            :to="{
              name: 'explore',
              query: { collections: this.$route.params.tokenContract },
            }"
          >
            <f-ellipsis
        ...
        <div class="nftdetailinfo_row">
          ...
          <span class="nftdetailinfo_tokenid">
            <f-ellipsis
              :text="toBigNumber(info.tokenId).toString(10)"
              overflow="middle"
            />
            <span>/</span>
        ...
        <div class="nftdetailinfo_row">
          ...
          <span>{{ $t('nftdetail.gtonNetwork') }}</span>
        </div>
        <div class="nftdetailinfo_row">
