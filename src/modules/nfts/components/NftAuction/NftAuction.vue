        ...
        <div class="nftauction_label grid">
            ...
            <template v-else>
              <div>
                ...
                <template v-if="!showEndCountdown"
                  >{{ auctionEndsIn }} ({{ auctionEndTime }})</template
                >
          ...
          <template v-else>
            <div>
              ...
              <span v-if="!showStartCountdown"
                >{{ auctionStartsIn }} ({{ auctionStartTime }})</span
              >
      ...
      <div class="grid">
        ...
        <template v-if="auctionHasStarted">
          <div class="flex gap-3 ali-center">
            ...
            />
            <span v-else>-</span>
            <span v-if="userIsLastBidder"
              >({{ $t('nftauction.youAreTopBidder') }})</span
            >
          </div>
          <div class="flex gap-3 ali-center" v-if="dAuction.lastBid">
            ...
            <router-link
              :to="{
                name: 'account',
                params: { address: dAuction.lastBidderUser.address },
              }"
            >
              <a-address
...
export default {
  ...
  computed: {
      ...
      return (
        ...
        (this.userOwnsToken &&
          ...
          (this.dAuction.reservePriceExceeded ||
            this.dAuction.props.sellerCanResultUnderpriced)) ||
    ...
    showStartCountdown() {
      return (
        !this.auctionHasStarted &&
        dayjs(this.dAuction.startTime).diff(dayjs(), 'days') < 3
      );
    },
    ...
    showEndCountdown() {
      return (
        this.auctionHasStarted &&
        dayjs(this.dAuction.endTime).diff(dayjs(), 'days') < 3
      );
    },
  },
