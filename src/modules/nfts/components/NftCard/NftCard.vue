<template>
  <div class="nftcard">
    <router-link
      :to="{
        name: 'nft-detail',
        params: { tokenContract: nftData.contract, tokenId: nftData.tokenId },
      }"
    >
      <div class="nftcard_content">
        <div class="nftcard_top">
          <div class="nftcard_itemName">
            <h3 class="nftcard_name" :data-tooltip="nftData.name">
              {{ nftData.name }}
            </h3>
            <div
              v-if="nftData.collection"
              class="nftcard_label"
              :data-tooltip="nftData.collection.name"
            >
          ...
          <div
            v-if="nftData.auctionReservePrice || nftData.auctionedPrice"
            class="nftcard_price"
          >
            <div class="nftcard_label">
              {{
                !nftData.auctionedPrice
                  ? $t('nftcard.minBid')
                  : $t('nftcard.topBid')
              }}
            </div>
          ...
          <div
            v-else-if="
              nftData.listingPrice ||
                nftData.offeredPrice ||
                nftData.lastTradePrice
            "
            class="nftcard_price"
          >
            ...
            <div v-if="nftData.offeredPrice" class="nftcard_offerprice">
              <span class="nftcard_offerprice_label">{{
                $t('nftcard.offerFor')
              }}</span>
              <a-token-value
            ...
            <div v-if="nftData.lastTradePrice" class="nftcard_lastprice">
              <span class="nftcard_lastprice_label">{{
                $t('nftcard.lastPrice')
              }}</span>
              <a-token-value
      ...
      <div class="nftcard_image">
        <div class="nftcard_box">
          <f-image
            size="100%"
            :src="getImageThumbUrl(nftData.imageThumb)"
            :alt="nftData.name"
          />
        </div>
      </div>
      <div class="nftcard_header">
        <button
          v-if="showBanButton"
          :data-tooltip="$t('nftcard.banUnban')"
          @click.prevent="onBanClick"
        >
