<template>
  <div class="flex gap-3 ali-center">
    ...
    <span v-if="withdrawBidButtonDisabled">
      ({{ $t('nftauction.canWithdrawIn', { in: withdrawBidTime }) }})
    </span>
    <span v-if="canResultFailedAuction">
      ({{ $t('nftauction.reserveNotMetCanWithdrawAnytime') }})
    </span>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    canResultFailedAuction() {
      return (
        !this.auction.reservePriceExceeded && this.auction.props.hasResultFailed
      );
    },
  ...
  methods: {
    withdrawBid() {
      ...
      if (this.canResultFailedAuction) {
        ...
        this.tx = contracts.resultFailedAuction(
          token.contract,
          token.tokenId,
          web3,
        ...
        this.tx = contracts.withdrawAuctionBid(
          token.contract,
          token.tokenId,
          web3,
          this.auction.auctionHall
