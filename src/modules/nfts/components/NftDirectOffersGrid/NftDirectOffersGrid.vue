      ...
      <template #column-actions="{ item }">
        <template v-if="actionExists(item)">
          <a-button
            v-if="canAccept(item)"
            :loading="
              txStatus === 'pending' && pickedAddress === item.proposedBy
            "
            :label="$t('nftDirectOffersGrid.accept')"
            :disabled="tokenHasAuction"
            :data-tooltip="
              tokenHasAuction ? $t('nftDirectOffersGrid.cantAccept') : null
            "
...
export default {
  ...
  methods: {
    ...
    setApproveTx(offer) {
      const { token } = this;
      const tx = erc721utils.erc721Approve(
        token.contract,
        offer.marketplace,
        token.tokenId
      );
    ...
    setAcceptOfferTx(offer) {
      const { token } = this;
      const web3 = new Web3();
      const tx = contracts.acceptOffer(
        token.contract,
        token.tokenId,
        offer.proposedBy,
        web3,
    ...
    withdrawOffer(offer) {
      const { token } = this;
      const web3 = new Web3();
      const tx = contracts.cancelOffer(
        token.contract,
        token.tokenId,
        web3,
        offer.marketplace
     ...
     */
    canAccept(offer) {
      const isProposer = this.compareAddresses(
        offer.proposedBy,
        this.walletAddress
      );
     ...
     */
    canWithdraw(offer) {
      const canWithdraw = this.compareAddresses(
        offer.proposedBy,
        this.walletAddress
      );
