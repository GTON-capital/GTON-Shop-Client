<template>
  <f-form
    ...
    :aria-label="
      !update
        ? $t('nftstartauctionwindow.title')
        : $t('nftupdateauctionwindow.title')
    "
  >
    <f-form-input
      ref="priceField"
      ...
      <a-button type="submit" size="large" :loading="txStatus === 'pending'">
        {{
          !update
            ...
            : $t('nftstartauctionform.updateAuction')
        }}
      </a-button>
    </div>
  ...
  methods: {
    ...
    async createAuction(values) {
      ...
      const reservePrice = values.reservePrice
        ? toHex(
            bToTokenValue(values.reservePrice, this.selectedPayToken.decimals)
          )
        : '0x0';
    ...
    async updateAuction(values) {
      ...
      if (values.reservePrice !== initialValues.reservePrice) {
        transactions.push(
          this.getUpdateAuctionReservePriceTx(values.reservePrice)
        );
      }
    ...
    setValues(auction, selectedPayToken) {
      ...
      if ('decimals' in selectedPayToken) {
        values.reservePrice = bFromTokenValue(
        ...
        this.startTimeDisabled =
          dayjs(auction.startTime).valueOf() < Date.now();
      }
    },
    ...
    getUpdateAuctionReservePriceTx(reservePrice) {
      ...
      const tx = contracts.updateAuctionReservePrice(
        ...
        token.tokenId,
        reservePrice
          ? toHex(bToTokenValue(reservePrice, this.selectedPayToken.decimals))
          : '0x0',
        web3,
    ...
    reservePriceValidator(value) {
      ...
      if (isNaN(val) || val <= 0) {
        ...
        this.updateInEscrow &&
        toBigNumber(
          bToTokenValue(val, this.selectedPayToken.decimals)
        ).isGreaterThan(this.auction.reservePrice)
      ) {
    ...
    endTimeValidator(value) {
      ...
      if (isNaN(ts)) {
        ...
        !this.update &&
        ts - tsStartTime > MAX_AUCTION_DURATION * 1000
      ) {
    ...
    onTransactionStatus(payload) {
      ...
      this.$emit('transaction-status', {
        ...payload,
        transactionsLeft: transactions.length,
      });
