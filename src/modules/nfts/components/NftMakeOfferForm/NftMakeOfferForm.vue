  ...
  methods: {
    async init() {
      ...
      if (this.listing.payToken) {
        this.selectedPayToken = await getPayToken(
          this.listing.payToken,
          this.payTokens
        );
      } else {
    ...
    deadlineValidator(value) {
      ...
      return dayjs(value).valueOf() <= now
        ? this.$t('nftMakeOfferForm.badDate')
        : '';
    },
    ...
    onTransactionStatus(payload) {
      ...
      if (this.txStatus === 'success') {
        if (txCode === 'create_offer_allowance') {
          this.setCreateOfferTx(
            this.storedValues.price,
            this.storedValues.deadline
          );
