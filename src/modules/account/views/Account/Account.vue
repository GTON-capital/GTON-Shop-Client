  ...
  <div class="account">
    ...
    <div class="account_header">
      <div
        ...
        :class="{ 'account_avatar-noavatar': !avatar }"
      >
        <AUploadArea
        ...
        <div
          v-if="!avatar"
          v-html="getJazzicon(userAddress, 118)"
          class="account_avatar_jazzicon"
        ></div>
      </div>
      <div class="account_title">
        {{ user.username || $t('account.unnamed') }}
      </div>
      ...
      <div class="account_join">
        <AccountFollow :user-address="userAddress" />
      </div>
  ...
  computed: {
    ...
    accountUserIsModerator() {
      return (
        ...
        compareAddresses(this.userAddress, this.walletAddress)
      );
    },
  },
  ...
  methods: {
    ...
    updateModeratorTabs() {
      ...
      onlyModeratorRoutes.forEach(route => {
        ...
        const navItemIdx = navigation.findIndex(
    ...
    async loadUser(userAddress) {
      if (userAddress) {
        ...
        this.avatar = this.user.avatarThumb
          ? getImageThumbUrl(this.user.avatarThumb)
          : '';
    ...
    setMetaInfo(user = this.user) {
      ...
      documentMeta.setMetaInfo({
        title: `${sTitle[0]} | ${this.$t('account.account')} ${user.username ||
          user.address}`,
      });
    },
    ...
    async updateTabsCounters() {
      ...
      if (tokenCounters.createdTokens) {
        accountNavigation.updateCounter(
          'account-created',
          toInt(tokenCounters.createdTokens.totalCount)
        );
      }
      if (tokenCounters.tokenLikes) {
        accountNavigation.updateCounter(
          'account-favorited',
          toInt(tokenCounters.tokenLikes.totalCount)
        );
      }
      if (tokenCounters.ownerships) {
        accountNavigation.updateCounter(
          'account-single-items',
          toInt(tokenCounters.ownerships.totalCount)
        );
      }
      if (tokenCounters.activities) {
        accountNavigation.updateCounter(
          'account-activity',
          toInt(tokenCounters.activities.totalCount)
        );
      }
      if (tokenCounters.offers) {
        accountNavigation.updateCounter(
          'account-offers',
          toInt(tokenCounters.offers.totalCount)
        );
      }
      if (tokenCounters.myOffers) {
        accountNavigation.updateCounter(
          'account-my-offers',
          toInt(tokenCounters.myOffers.totalCount)
        );
      }
    },
    ...
    async updateBannedTokensCounter() {
      ...
      accountNavigation.updateCounter(
        'account-banned-tokens',
        toInt(bannedTokenCounters.totalCount)
      );
    },
