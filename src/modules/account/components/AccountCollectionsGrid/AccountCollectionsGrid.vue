  ...
  <div class="accountcollectionsgrid">
    ...
    >
      ...
      <template #column-ownerUser="{ value }">
        <router-link
          :to="{ name: 'account', params: { address: value.address } }"
        >
          <a-address
            :address="value.address"
            :name="value.username"
            :image="value.avatarThumb"
            is-account
          />
        </router-link>
      </template>
      ...
      <template #column-actions="{ item }">
        ...
        >
          <app-iconset icon="ban" size="16px" />
          {{ $t('accountcollectionsgrid.ban.button') }}
        </a-button>
        <a-button
          v-if="filter === 'banned'"
          :loading="item._loading"
        ...
        >
          <app-iconset icon="thumb-up" size="16px" />
          {{ $t('accountcollectionsgrid.unban.button') }}
        </a-button>
        <template v-else-if="filter === 'inReview'">
          ...
          >
            <app-iconset icon="thumb-up" size="16px" />
            {{ $t('accountcollectionsgrid.approve.button') }}
          </a-button>
          <a-button
            :loading="item._loading"
            :disabled="item._disabled"
          ...
          >
            <app-iconset icon="thumb-down" size="16px" />
            {{ $t('accountcollectionsgrid.decline.button') }}
          </a-button>
        </template>
      </template>
    </f-data-grid>
...
import {
  banCollection,
  unbanCollection,
} from '@/modules/collections/mutations/ban-unban.js';
import {
  approveCollection,
  declineCollection,
} from '@/modules/collections/mutations/approve-decline.js';
...
export default {
  ...
  data() {
    return {
      ...
      filterData: [
        {
          value: 'enabled',
          label: this.$t('accountcollectionsgrid.filterEnabled'),
        },
        {
          value: 'banned',
          label: this.$t('accountcollectionsgrid.filterBanned'),
        },
        {
          value: 'inReview',
          label: this.$t('accountcollectionsgrid.filterInReview'),
        },
      ],
    };
  },
  ...
  methods: {
    ...
    async loadPage(pagination = { first: this.perPage }) {
      return await this.getCollectionsByFilter(
        this.filter,
        this.searchText,
        pagination
      );
    ...
    async action(mutation, collection, message) {
      if (
        confirm(
          this.$t('accountcollectionsgrid.' + message + '.confirm', collection)
        )
      ) {
