<template>
  <div class="pg-nft">
    ...
    <div class="pg-nft__text">
      ...
      <h5 class="h5" v-if="token.hasAuction">
        This unique NFT is redeemable for:
      </h5>
      ...
      <div class="pg-nft__socials mat-7">
        ...
        <span class="pg-nft__socials-button">
          <a
            href="https://twitter.com/PierreGASLY"
            target="_blank"
            class="btn btn-lg btn-round btn-samesize"
          >
            <icon-twitter-white />
  ...
  computed: {
    minBidAmount() {
      ...
      return auction.minBidAmount
        ? formatTokenValue(auction.minBidAmount, this.payToken.decimals, 1)
        : '-';
    },
  ...
  methods: {
    ...
    async loadAuction() {
      ...
      if (this.auctionOn && token.hasAuction) {
        ...
        if (
          auction &&
          JSON.stringify(this.auction) !== JSON.stringify(auction)
        ) {
