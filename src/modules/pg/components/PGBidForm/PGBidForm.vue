<template>
  <div class="pg-bid-form">
    <div
      class="pg-bid-form__current-bid"
    ...
    <div class="pg-bid-form__my-bid">
      ...
      <input
        ...
        :class="{
          'pg-bid-form__amount--zero': !amount,
          'pg-bid-form__amount--error': error,
        }"
        :disabled="txStatus === 'pending'"
        type="number"
        v-model="amount"
...
export default {
  ...
  computed: {
    ...
    minNextBidB() {
      ...
      return bToTokenValue(
        this.fromWeiToNumber(this.minBidAmountB) + this.minIncrement,
        this.payToken.decimals
      );
    },
  },
  ...
  methods: {
    async init() {
      ...
      this.amount = parseInt(
        bFromTokenValue(this.minNextBidB, this.payToken.decimals).toNumber()
      );
    },
    ...
    async setTx(_amountB) {
      ...
      const { auction } = this;
      let amountB =
        _amountB || bToTokenValue(this.amount, this.payToken.decimals);
    ...
    setAllowanceTx(_amountB) {
      ...
      const tx = erc20Utils.erc20IncreaseAllowanceTx(
        ...
        process.env.VUE_APP_GTON_AUCTION_CONTRACT_ADDRESS,
    ...
    async updateUserBalance() {
      this.userBalanceB = toBigNumber(
        await getErc20TokenBalance(this.walletAddress, this.payToken.address)
      );
