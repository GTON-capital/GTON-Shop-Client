...
const TOKENS = [
  {
    ...
    hasAuction: true,
    text:
      'Azerbaijan 2021 race-worn helmet AND<br>Meet and greet with Pierre over the next 12 months',
  ...
  methods: {
    ...
    async loadMPayTokens() {
      const prices = await Promise.all(
        PAY_TOKENS.map(payToken =>
          getRandomTradePayTokenPrice(RANDOM_TRADE_CONTRACT, payToken.address)
        )
      );
