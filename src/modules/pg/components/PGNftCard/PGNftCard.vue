<template>
  <div class="pg-nft-card">
    <div
      class="pg-nft-card__img-cont"
      :class="{ 'pg-nft-card__img-cont--limited': !token.hasAuction }"
    >
    ...
    <div class="pg-nft-card__cta">
      <div class="pg-nft-card__cta-top">
        <div class="pg-nft-card__bid">
          ...
          <template v-else-if="!auctionOn">
            ...
            <h4 class="h4">
              {{ formatTokenValue(token.startingPrice, payToken.decimals) }}
              wFTM
            </h4>
        ...
        <div class="pg-nft-card__countdown">
          ...
          <template v-else>
            <h6 class="h6">
              {{
                auctionOn ? $t('pgNftCard.endsIn') : $t('pgNftCard.startsIn')
              }}
            </h6>
      ...
      <div
        class="pg-nft-card__address"
        :class="{ 'pg-nft-card__address--empty': !lastBidder }"
      >
      ...
      <div class="pg-nft-card__cta-bottom">
        ...
        <template v-else-if="!sold">
          <span
            v-for="pt in mPayTokens"
            :key="`pt_${pt.address}`"
            class="pg-nft-card__button mab-2"
          >
            <a-button
    ...
    <a-sign-transaction
      :tx="buyTx"
      @transaction-status="onBuyTransactionStatus"
    />
  </div>
</template>
...
const RANDOM_TRADE_CONTRACT =
  process.env.VUE_APP_GTON_RANDOM_PURCHASE_CONTRACT_ADDRESS;
...
export default {
  ...
  computed: {
    ...
    currentBid() {
      ...
      return lastBid
        ? formatTokenValue(lastBid, this.payToken.decimals, 1)
        : '';
    },
  ...
  methods: {
    ...
    to$(value) {
      const value$ = value
        ? toBigNumber(value).multipliedBy(this.payToken.price)
        : null;

      return value$
        ? formatTokenValue(value$, this.payToken.priceDecimals, 2, true)
        : '';
    },
    ...
    setBuyTx() {
      const web3 = new Web3();
      const tx = contracts.randomPurchase(
    ...
    setBuyAllowanceTx(tokenPriceB) {
      ...
      console.log(
        'set allowence',
        bFromTokenValue(tokenPriceB, 18).toNumber(),
        toHex(tokenPriceB.plus(10))
      );
    ...
    async buyMToken(payToken) {
      ...
      if (tokenPriceB.isGreaterThan(userBalanceB)) {
        this.$notifications.add({
          text: this.$t('pgNftCard.insufficientBalance', {
            token: payToken.label,
          }),
          type: 'error',
        });
      ...
      const userAllowanceB = await this.getUserAllowance(
        payToken.address,
        RANDOM_TRADE_CONTRACT
      );
    ...
    async getUserBalance(payTokenAddress = '') {
      return toBigNumber(
        await getErc20TokenBalance(this.walletAddress, payTokenAddress)
      );
    },
